...
func upgradeTabMultChange(mult : int, currencyIndex : int, money : float) -> void:
	for i in range(upgMax):
		if upgradeCurrencies[i] == currencyIndex:
			upgradeStatus[i] = get_child(UPGRADE_INDEX_OFFSET + i).multChange(mult, money)
		...
		for i in range(upgMax):
			...
			if upgradeCurrencies[i] == currency:
				...
				if gain and upgradeStatus[i] >= 1:
					upgradeStatus[i] = get_child(UPGRADE_INDEX_OFFSET + i).posChange(money)
				elif not gain and upgradeStatus <= 1:
					upgradeStatus[i] = get_child(UPGRADE_INDEX_OFFSET + i).negChange(money)
...
func _createPanel(global_pos : Vector2, col : Color) -> void:
	...
	connect(tempPanel.crumpled, _on_panel_container_crumpled)
...
func toggleMaxedUpgrades() -> void:
	for pos in completedUpgrades:
		get_child(UPGRADE_INDEX_OFFSET + pos).visible = maxedUpgradesHidden
