...
func attack(temp : float, charge_cd : float) -> void:
	...
	if not attacking and (rustL or rustR):
		...
		var bladeLen = max(2.0 * temp/5.0, 1.5) 
...
func _colShapeIncrease(BL : int, inc : float) -> void:
	var tempShape = CircleShape2D
	var abs_size = size * inc
	match BL:
		0:
			tempShape.radius = 17 * abs_size
			$Hitbox/CollisionShape2D.set_deferred("shape", tempShape)
			$Hitbox/CollisionShape2D.set_deferred("position:y", -20*abs_size)
		1:
			tempShape.radius = 24 * size
			$Hitbox/CollisionShape2D.set_deferred("shape", tempShape)
			$Hitbox/CollisionShape2D.set_deferred("position:y", -22*abs_size)
		2:
			tempShape.radius = 28 * size
			$Hitbox/CollisionShape2D.set_deferred("shape", tempShape)
			$Hitbox/CollisionShape2D.set_deferred("position:y", -32*abs_size)

func _colShapeReset(BL : int) -> void:
	var tempShape = CircleShape2D
	match BL:
		0:
			tempShape.radius = 17 * size
			$Hitbox/CollisionShape2D.set_deferred("shape", tempShape)
			$Hitbox/CollisionShape2D.set_deferred("position:y", -20*size)
		1:
			tempShape.radius = 24 * size
			$Hitbox/CollisionShape2D.set_deferred("shape", tempShape)
		...
		2:
			tempShape.radius = 28 * size
			$Hitbox/CollisionShape2D.set_deferred("shape", tempShape)
			$Hitbox/CollisionShape2D.set_deferred("position:y", -32*size)
...
func _setSize() -> void:
	...
	if attacking:
		...
		$Detection/CollisionShape2D.set_deferred("position:y", -20*size)
		...
		tempShape.radius = 28 * size
		$Detection/CollisionShape2D.set_deferred("shape", tempShape)
		$Detection/CollisionShape2D.set_deferred("position:y", -32*size)
	else:
		var tempShape = CircleShape2D
		tempShape.radius = 17 * size
		$Hitbox/CollisionShape2D.set_deferred("shape", tempShape)
	...
	$HitboxL/CollisionShape2D.set_deferred("shape", tempRect)
	$HitboxL/CollisionShape2D.set_deferred("position", Vector2(-18,-23)*size)
	$HitboxR/CollisionShape2D.set_deferred("shape", tempRect)
	$HitboxR/CollisionShape2D.set_deferred("position", Vector2(18,-23)*size)
	
	tempRect.size = size * Vector2(20.0, 60.0)
	$HitboxL/CollisionShape2D.set_deferred("shape", tempRect)
	$HitboxL/CollisionShape2D.set_deferred("position", Vector2(-20,-30)*size)
	$HitboxR/CollisionShape2D.set_deferred("shape", tempRect)
	$HitboxR/CollisionShape2D.set_deferred("position", Vector2(20,-30)*size)
...
func _deRustLeft() -> void:
	var rust_length = 7.0 / rust_recovery_speed * min(rust_level_l / rust_max, 1.0)
...
func _deRustRight() -> void:
	var rust_length = 7.0 / rust_recovery_speed * min(rust_level_r / rust_max, 1.0)
